...
class REDQAgentV2(object):
    def __init__(
        ...
        for i in range(self.N):
            ...
            critic = Critic(state_size, action_size, hidden_size, i).to(self.device)
            self.critic_params = self.critic_params + list(critic.parameters())
        ...
        self.critic_optimizer = optim.Adam(self.critic_params, lr=lr_c, eps=1e-5)
    ...
    def save_model(self, save_path, is_best=True):
        ...
        if is_best:
            ...
            for i in range(self.N):
                ...
                torch.save(self.critic_list[i].state_dict(), os.path.join(save_path, f"redq_v2_critic_{i}_best.pth"))
            
            torch.save(self.actor.state_dict(), os.path.join(save_path, "redq_v2_actor_best.pth"))

        else:

            for i in range(self.N):
                
                torch.save(self.critic_list[i].state_dict(), os.path.join(save_path, f"redq_v2_critic_{i}_end.pth"))
            
            torch.save(self.actor.state_dict(), os.path.join(save_path, "redq_v2_actor_end.pth"))
    ...
    def load_model(self, load_path, is_best=True):
        ...
        if is_best:
            ...
            for i in range(self.N):
                ...
                self.critic_list[i].load_state_dict(torch.load(os.path.join(load_path, f"redq_v2_critic_{i}_best.pth")))

            self.actor.load_state_dict(torch.load(os.path.join(load_path, "redq_v2_actor_best.pth")))
        else:
        
            for i in range(self.N):

                self.critic_list[i].load_state_dict(torch.load(os.path.join(load_path, f"redq_v2_critic_{i}_end.pth")))

            self.actor.load_state_dict(torch.load(os.path.join(load_path, "redq_v2_actor_end.pth")))